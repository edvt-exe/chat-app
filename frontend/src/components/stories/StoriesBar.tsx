...
export default function StoriesBar() {
  ...
  const grouped = stories.reduce((acc, story) => {
    ...
    return acc;
  }, {} as Record<string, { user: any; stories: typeof stories }>);

  return (
    <div className="flex items-center gap-3 px-4 py-3 overflow-x-auto"
      style={{ borderBottom: '1px solid rgba(0,200,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>

      <button onClick={handleAddStory} className="flex flex-col items-center gap-1.5 flex-shrink-0">
        <div className="w-11 h-11 rounded-full flex items-center justify-center text-lg font-semibold transition-colors"
          style={{
      ...
      {Object.values(grouped).map(({ user: storyUser, stories: userStories }) => {
        ...
        return (
          <button key={storyUser.id} className="flex flex-col items-center gap-1.5 flex-shrink-0">
            <div
            ...
            >
              <div className="w-9 h-9 rounded-full flex items-center justify-center overflow-hidden"
                ...
                {storyUser.avatarUrl ? (
                  <img src={storyUser.avatarUrl} className="w-full h-full object-cover" alt="" />
                ) : (
